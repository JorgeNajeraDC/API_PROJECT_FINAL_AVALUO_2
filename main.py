...
def generar_token(client_id: str, secret: str) -> str:
    payload = {'clientId': client_id}
    token_bytes = jwt.encode(payload, secret, algorithm='HS256')
    return token_bytes

@app.post("/token_avaluo/")
def obtener_token(user: User):
    global token

    if token is not None:
        return {
            "message": "Ya se generó un token anteriormente"}  # Manejar el caso de que ya se haya generado un token anteriormente

    client_id = user.clientId
    secret = user.secret

    # Obtener los valores del archivo .env
    CLIENTID_BPB = os.environ.get("CLIENTID_BPB")
    SECRET = os.environ.get("SECRET")

    if secret == SECRET and client_id == CLIENTID_BPB:
        token = generar_token(client_id, secret)
